...
public sealed class ApiHandler
{
    ...
    public void MapRoutes(WebApplication app)
    {
        ...
        // Live-motion edge stats (zinc-in-motion)
    ...
    private IResult AddProcessor([FromBody] Dictionary<string, object?>? req)
    {
        ...
        return Results.Content(
            ZincJson.SerializeToString(new Dictionary<string, object?> { ["error"] = err ?? "add failed" }),
    ...
    private IResult Readyz()
    {
        ...
        if (ready)
            return Results.Content(ZincJson.SerializeToString(body), "application/json", statusCode: 200);
        return Results.Content(ZincJson.SerializeToString(body), "application/json", statusCode: 503);
    }
    ...
    /// Wrap ZincJson.SerializeToString (AOT-safe source-gen) with a
    ...
    private static IResult JsonStatus(int status, object value)
        => Results.Content(ZincJson.SerializeToString(value), "application/json", null, status);
