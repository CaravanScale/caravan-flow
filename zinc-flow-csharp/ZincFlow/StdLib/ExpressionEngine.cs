...
{
    private readonly ZincFlow.Core.Record _record;
    public RecordValueResolver(ZincFlow.Core.Record record) => _record = record;
    ...
    public EvalValue Resolve(string name)
    {
        ...
        for (int i = 1; i < parts.Length; i++)
        {
            switch (cur)
            {
                ...
                case ZincFlow.Core.Record gr: cur = gr.GetField(parts[i]); break;
