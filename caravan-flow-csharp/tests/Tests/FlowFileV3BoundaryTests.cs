...
public static class FlowFileV3BoundaryTests
{
    ...
    static void TestPutFileV3Mode()
    {
        ...
        var tmp = Path.Combine(Path.GetTempPath(), $"caravan-v3-{Guid.NewGuid():N}");
    ...
    static void TestPutFileRawMode()
    {
        ...
        var tmp = Path.Combine(Path.GetTempPath(), $"caravan-raw-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tmp);
        try
        {
    ...
    static void TestGetFileSniffsV3()
    {
        ...
        var tmp = Path.Combine(Path.GetTempPath(), $"caravan-getfile-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tmp);
        try
        {
    ...
    static void TestEndToEndV3FilePipelineRoundtrip()
    {
        ...
        var tmp = Path.Combine(Path.GetTempPath(), $"caravan-e2e-{Guid.NewGuid():N}");
