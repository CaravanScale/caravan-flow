...
public static class PipelineTests
{
    ...
    static void TestPipelineLargeContentOffload()
    {
        ...
        var store = new FileContentStore("/tmp/caravan-flow-csharp-test-large");
        var cleanup = new ContentStoreCleanup(store, "/tmp/caravan-flow-csharp-test-large");
        ...
        {
            if (Directory.Exists("/tmp/caravan-flow-csharp-test-large"))
                Directory.Delete("/tmp/caravan-flow-csharp-test-large", true);
        }
    }
