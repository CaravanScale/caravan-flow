...
public static class SourceTests
{
    ...
    static void TestPutFile()
    {
        ...
        var tmpDir = Path.Combine(Path.GetTempPath(), $"caravan-test-putfile-{Environment.TickCount64}");
    ...
    static void TestConnectorSourceLifecycle()
    {
        ...
        var tmpDir = Path.Combine(Path.GetTempPath(), $"caravan-test-lifecycle-{Environment.TickCount64}");
        Directory.CreateDirectory(tmpDir);
        try
        {
    ...
    static void TestGetFile()
    {
        ...
        var tmpDir = Path.Combine(Path.GetTempPath(), $"caravan-test-getfile-{Environment.TickCount64}");
        Directory.CreateDirectory(tmpDir);
        try
        {
            var store = new MemoryContentStore();
    ...
    static void TestContentStoreCleanup()
    {
        ...
        var tmpDir = Path.Combine(Path.GetTempPath(), $"caravan-test-cleanup-{Environment.TickCount64}");
        try
        {
