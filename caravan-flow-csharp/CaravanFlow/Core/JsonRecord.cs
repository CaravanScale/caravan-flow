...
public sealed class JsonRecordReader : IRecordReader
{
    ...
    {
        try { result = JsonSerializer.Deserialize(data, CaravanJsonContext.Default.ListDictionaryStringObject); return true; }
        catch { result = null; return false; }
    }

    private static bool TryDeserializeObject(byte[] data, out Dictionary<string, object?>? result)
    {
        try { result = JsonSerializer.Deserialize(data, CaravanJsonContext.Default.DictionaryStringObject); return true; }
        catch { result = null; return false; }
    }
}
