...
public sealed class MetricsHandler
{
    ...
    public Task HandleMetrics(HttpContext ctx)
    {
        ...
        sb.AppendLine("# HELP caravan_flow_processed_total Total FlowFiles processed");
        sb.AppendLine("# TYPE caravan_flow_processed_total counter");
        sb.Append("caravan_flow_processed_total ").AppendLine(stats.GetValueOrDefault("processed", (object)0)?.ToString() ?? "0");
        ...
        sb.AppendLine("# HELP caravan_flow_processor_processed_total FlowFiles processed per processor");
        sb.AppendLine("# TYPE caravan_flow_processor_processed_total counter");
        foreach (var (name, ps) in procStats)
            sb.Append("caravan_flow_processor_processed_total{processor=\"").Append(name).Append("\"} ").AppendLine(ps.GetValueOrDefault("processed", 0).ToString());

        sb.AppendLine("# HELP caravan_flow_processor_errors_total Errors per processor");
        sb.AppendLine("# TYPE caravan_flow_processor_errors_total counter");
        foreach (var (name, ps) in procStats)
            sb.Append("caravan_flow_processor_errors_total{processor=\"").Append(name).Append("\"} ").AppendLine(ps.GetValueOrDefault("errors", 0).ToString());

        // Source connectors
        var sources = _fab.GetSources();
        sb.AppendLine("# HELP caravan_flow_source_running Whether connector source is running (1/0)");
        sb.AppendLine("# TYPE caravan_flow_source_running gauge");
        foreach (var src in sources)
            sb.Append("caravan_flow_source_running{name=\"").Append(src.Name).Append("\",type=\"").Append(src.Type).Append("\"} ").AppendLine(src.Running ? "1" : "0");

        // Uptime
        sb.AppendLine("# HELP caravan_flow_uptime_seconds Time since process start");
        sb.AppendLine("# TYPE caravan_flow_uptime_seconds gauge");
        sb.Append("caravan_flow_uptime_seconds ").AppendLine(((Environment.TickCount64 - _startTick) / 1000.0).ToString("F1"));
