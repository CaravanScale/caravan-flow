...
caravan-flow is heading to Kubernetes. Each worker pod runs the engine
...
- **Worker** (current `caravan-flow`): engine + management API + metrics.
  No UI. Scales horizontally as a K8s Deployment.
- **UI** (new `caravan-flow-ui`): tiny ASP.NET host serving a Blazor
...
```
+----------------------+         +----------------------+
| caravan-flow-ui (NEW)   |         | caravan-flow (worker 1) |
...
|   - K8s discovery    |
|     (later)          |         +----------------------+
| Aggregated views     |--HTTP-->| caravan-flow (worker N) |
| YAML write-back      |         +----------------------+
| Git integration      |
+----------------------+
```
...
2. **Worker self-registration** (opt-in per worker). Worker config:
   `ui.register_to: http://caravan-flow-ui.ns.svc:9090`. Worker POSTs
...
New sibling project `caravan-flow-ui-csharp/`. Targets a single worker
