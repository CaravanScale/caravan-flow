...
final class MetricsTest {
    ...
    void scrapeContainsProcessorCounter() {
        ...
        assertTrue(scrape.contains("caravanflow_ingested_total"),
                "expected caravanflow_ingested_total in metrics, got:\n" + scrape);
        assertTrue(scrape.contains("processor=\"noop\""),
                "expected per-processor tag in scrape output, got:\n" + scrape);
        assertTrue(scrape.contains("caravanflow_dropped_total 2"),
    ...
    void statsAndMetricsAgree() {
        ...
        assertTrue(metrics.scrape().contains("caravanflow_ingested_total 5"),
    ...
    void defaultPipelineHasMetrics() {
        ...
        assertTrue(pipeline.metrics().scrape().contains("caravanflow_ingested_total 1"));
    }
}
