...
final class MetricsTest {
    ...
    void scrapeContainsProcessorCounter() {
        ...
        assertTrue(scrape.contains("caravan_flow_ingested_total"),
                "expected caravan_flow_ingested_total in metrics, got:\n" + scrape);
        assertTrue(scrape.contains("processor=\"noop\""),
                "expected per-processor tag in scrape output, got:\n" + scrape);
        assertTrue(scrape.contains("caravan_flow_dropped_total 2"),
    ...
    void statsAndMetricsAgree() {
        ...
        assertTrue(metrics.scrape().contains("caravan_flow_ingested_total 5"),
    ...
    void defaultPipelineHasMetrics() {
        ...
        assertTrue(pipeline.metrics().scrape().contains("caravan_flow_ingested_total 1"));
    }
    ...
    void uptimeGaugeIsExposed() {
        ...
        var metrics = new Metrics();
        String scrape = metrics.scrape();
        assertTrue(scrape.contains("caravan_flow_uptime_seconds"),
                "expected caravan_flow_uptime_seconds gauge in scrape, got:\n" + scrape);
    }
    ...
    void activeExecutionsGaugeSettlesAtZero() {
        ...
        String scrape = metrics.scrape();
        assertTrue(scrape.contains("caravan_flow_active_executions 0"),
    ...
    void activeExecutionsReflectsMidFlightIngest() {
        ...
        assertTrue(midScrape.contains("caravan_flow_active_executions 1"),
        ...
        assertTrue(metrics.scrape().contains("caravan_flow_active_executions 0"),
    ...
    void sourceRunningGaugeTracksSource() {
        ...
        assertTrue(stoppedScrape.contains("caravan_flow_source_running"),
        ...
        assertTrue(runningScrape.matches("(?s).*caravan_flow_source_running\\{[^}]*\\} 1(\\.0)?.*"),
