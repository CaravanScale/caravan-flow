...
# zinc-csharp's sibling so the React UI in zinc-flow-ui-web
...
class ApiServer
  ...
  # Fabric-wide stats mirroring zinc-csharp's GET /api/stats shape.
  ...
  private def prometheus(ctx) : Nil
    ...
    @fabric.stats.each do |s|
      ctx.response.puts "zinc_processed_total{node=\"#{s[:name]}\",type=\"#{s[:type]}\",state=\"#{s[:state]}\"} #{s[:processed]}"
      ctx.response.puts "zinc_errors_total{node=\"#{s[:name]}\",type=\"#{s[:type]}\"} #{s[:errors]}"
    end
    EdgeStats.snapshot.each do |k, v|
      ...
      rel = parts[1]? || ""
      to = parts[2]? || ""
      ctx.response.puts "zinc_edge_processed_total{from=\"#{from}\",relationship=\"#{rel}\",to=\"#{to}\"} #{v[:processed]}"
    end
  end
