# caravan-flow — non-gov product research (April 2026 snapshot)

## Context

caravan-flow was built with a gov / NiFi-replacement shape in mind (IRS-style routing, airgapped schema registry, V3 FlowFile interop, provenance-grade compliance story). Now looking outward: what commercial product shapes can this engine power, outside government use? This file captures the market research so we can revisit and make a direction call without re-doing the survey.
...
- `caravan-flow validate` CLI (pre-flight config check)
...
- 200+ processors (vs caravan-flow's 24)
...
**Why caravan-flow fits:** airgapped schema registry built-in, provenance tracking, ScopedContext isolation, zero external deps, tiny binary, FlowFile V3 interop lets it coexist with existing NiFi during migration.
...
**Why caravan-flow fits:** 11 MB Go binary, no JVM, runs on Raspberry Pi / ARM64 edge boxes, handles structured data (Avro/JSON/CSV) not just syslog.
...
Observability SaaS that needs an in-agent pipeline. AI product needing a retrieval pipeline. Data science platform needing feature prep. Nango/Ampersand do **external-API integration** for SaaS; caravan-flow's angle is **internal data transformation** within the SaaS product.

**Why caravan-flow fits:** single static binary, YAML config, hot reload, multi-runtime (ship whichever matches host platform), AOT for minimal attack surface.
...
**Why caravan-flow fits:** Python 3.14t free-threading, pandas/numpy/sklearn native, 14 MB native binary via Cython, composable processor pattern lets data scientists chain transforms declaratively.

**What's missing:** Python runtime itself is Phase 1 (queue-based, old processor names). Needs the csharp-parity catch-up work first (~1 week per `caravan-flow-python/docs/csharp-parity-plan.md`).
...
**Why caravan-flow fits:** FlowFile model handles large binary payloads well (content offload to disk claims above 256 KB), routing is first-class.
...
`caravan-flow` as the `make` or `jq` of data pipelines — a local CLI that reads YAML and runs a pipeline, useful in CI/CD, scripts, local dev environments. Low-ticket / OSS / developer-love play.

**Why caravan-flow fits:** zero-dep binary, YAML, validate subcommand already exists, fast startup.
...
### Concept 1: caravan-edge — edge/IoT data pipeline OSS + commercial
...
### Concept 2: caravan-embed — embedded data-flow library for SaaS OEMs
...
### Concept 3: caravan-ml — AI/ML data prep platform (python-runtime-led)
...
**Risk:** Crowded space with deep-pocketed competitors. Needs strong opinion on what makes caravan-flow better than Dagster for ML.

### Concept 4: caravan-healthcare — FHIR/HL7 gateway for mid-market

**Shape:** Vertical-focused distribution. caravan-flow + HL7 v2 parser + FHIR REST + EHR integration connectors + HIPAA-shaped audit log + pre-packaged pipelines for common clinical workflows (lab results, ADT, orders). Self-hosted appliance model or hosted HITRUST-certified SaaS.
...
### Concept 5: caravan-cli — developer tooling play

**Shape:** Position caravan-flow as "`make` for data pipelines." OSS-only, developer love, maybe GitHub sponsors / consulting revenue. Focus on local dev UX, stdin/stdout mode, package-manager distribution, a cookbook site.
...
1. **caravan-edge (Concept 1)** — clear market, existing Go binary is already close, differentiation is defensible (structured-data edge routing, not logs), $28B market growing 28%. Edge/IoT buyers are used to paying for commercial support on OSS.

2. **caravan-embed (Concept 2)** — highest revenue-per-customer ceiling, fits the "invisible infrastructure" thesis that caravan-flow's design already supports (single binary, zero deps, AOT, multi-runtime). But slower to first revenue.

**Suggested path:**
- Start with caravan-edge as the public story (OSS + commercial support, brand-building, developer community). Lower risk, faster feedback.
- Use caravan-edge customer relationships to learn which verticals need caravan-embed-style embedding. Convert selectively.
- caravan-healthcare / caravan-ml are follow-ups once a first commercial motion exists; both have specialized sales cycles that need a base business to sustain them.

**Anti-recommendation:** don't pursue caravan-cli as primary. It's a good side-effect of any of the above, not a standalone product.
...
3. **Choose 1-2 connectors that unblock the target concept** (MQTT for caravan-edge, embedding SDK for caravan-embed, HL7 for caravan-healthcare)
