...
final class FakeWorker {
    ...
    static Map<String, Object> sampleOverlays() {
        ...
        base.put("base", "/etc/zinc/config.yaml");
        base.put("layers", java.util.List.of(
                Map.of("role", "base",    "path", "/etc/zinc/config.yaml",    "present", true,  "size", 3),
                Map.of("role", "local",   "path", "/etc/zinc/config.local.yaml","present", true,  "size", 1),
                Map.of("role", "secrets", "path", "/etc/zinc/secrets.yaml",   "present", false, "size", 0)));
