...
React UI (`zinc-flow-ui-web`) has shipped phases 1 / 2a / 2b / 2c / 2d (Graph CRUD + observability views). Multi-worker management is addressed by the operator + aggregator split in Phase 2g below.
...
- [ ] **B. Headless mode on worker** — `--mode=headless` + `ZINC_MODE` env var. Skip wwwroot, disable VC provider, 405 on `/api/flow/save`, mounted ConfigMap file watcher, hot reload on change.
- [ ] **C. Aggregator skeleton (classic mode)** — new `zinc-flow-aggregator-csharp/` project. Serves React bundle, implements full HTTP contract with fan-out over a static peer list, canonical config file + optional git integration. Enables docker-compose / VM multi-worker without k8s.
- [ ] **D. CRDs + operator skeleton** — new `zinc-flow-operator/` repo, Go + kubebuilder. Flow + FlowCluster CRDs, reconciler to ConfigMap + Deployment + Service, admission webhook, status subresource. E2E tests on kind.
...
- [ ] **F. Primary-node handling** — operator manages Lease per Flow, injects `ZINC_PRIMARY_NODE` env var. Worker respects `primaryOnly: true`. UI crown icon.
...
- [ ] **I. Packaging + docs** — Helm chart under `charts/zinc-flow/`, Kustomize bases under `deploy/kustomize/`, `zinc-flow migrate-to-crd` CLI, getting-started per mode, operator runbook.
...
- [ ] **NiFi-style dynamic layout, per-user persistence** — currently
      `zinc-flow-ui-web` forces dagre auto-layout (`nodesDraggable=false`)
